...
DATABASE_URL="${DATABASE_URL:-root:password@tcp(localhost:3306)/agent_marketplace?charset=utf8mb4&parseTime=True}"
...
tell application \"Terminal\"
  activate
  set backendTab to do script \"cd '$ROOT_DIR' && bash scripts/dev/dev.sh backend\"
...
tell application \"Terminal\"
  activate
  set frontendTab to do script \"cd '$ROOT_DIR' && bash scripts/dev/dev.sh frontend\"
