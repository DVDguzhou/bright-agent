 ...
 * 用法：
 *   # 列出所有 Agent
 *   node scripts/chat.mjs --list
 *
 *   # 跟某个 Agent 聊天（交互模式）
 *   node scripts/chat.mjs --agent <AGENT_ID>
 *
 *   # 单条消息模式（不进入交互）
 *   node scripts/chat.mjs --agent <AGENT_ID> --msg "考研数学怎么复习？"
 *
 * 环境变量：
...
async function main() {
  ...
  if (hasFlag("--list")) {
    ...
    console.log("使用：node scripts/chat.mjs --agent <ID>");
    return;
  }
  ...
  if (!agentId) {
    console.log("用法：");
    console.log("  node scripts/chat.mjs --list              列出所有 Agent");
    console.log("  node scripts/chat.mjs --agent <ID>        交互式聊天");
    console.log('  node scripts/chat.mjs --agent <ID> --msg "你好"  单条消息');
    return;
  }
