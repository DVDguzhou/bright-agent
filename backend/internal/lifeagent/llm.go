...
func twoPhaseLifeAgentReply(ctx context.Context, client *openai.Client, model string, apiKey, baseURL string, profile ProfileForAI, facts []StructuredFactForAI, topics []TopicSummaryForAI, entries []KnowledgeEntryForAI, history []ChatMessageForAI, message string, onChunk func(string), opts *ChatOptions) (content string, references []map[string]string, err error) {
	...
	if chatIntent != chatIntentSmallTalk {
		if opts != nil {
			AttachTimelineSourceEntries(&fullPlan, opts.TimelineEvents, entries, message)
		}
...
func buildReconcileSystemPrompt(profile ProfileForAI, catalog CitationCatalog, citationsEnabled bool, sparse bool) string {
	...
	if sparse {
		...
		sb.WriteString("3.1【防露馅 - 硬约束】凡草稿里出现【编号素材】没有写到的具体事实（地名、人名、机构、数字、日期、具体对话、具体事件经过），一律删掉，或改成模糊不确定的口语（如「记不太清了」「大概那样」「具体细节有点忘了」）；只有素材支撑得住的具体事实才保留。感受、态度、语气、口语不受此限——这条只锁「会被点开核对的具体事实」，不锁人味。\n")
	}
