...
func Setup(cfg *config.Config) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		auth := api.Group("/auth")
		{
			...
			auth.PATCH("/me", middleware.RequireAuth(cfg), handler.UpdateMe(cfg))
		...
		lifeAgents := api.Group("/life-agents")
		{
			...
			lifeAgents.POST("/feedback/mark-read", middleware.RequireAuth(cfg), handler.LifeAgentsFeedbackMarkRead(cfg))
			...
			lifeAgents.DELETE("/:id/timeline-events/:eventId", middleware.RequireAuth(cfg), handler.LifeAgentsTimelineEventDelete(cfg))
			lifeAgents.GET("/:id/mind-settings", middleware.Auth(cfg), handler.LifeAgentsMindSettingsGet(cfg))
			lifeAgents.PATCH("/:id/mind-settings", middleware.RequireAuth(cfg), handler.LifeAgentsMindSettingsPatch(cfg))
			lifeAgents.GET("/:id/citations/:sourceType/:sourceId", middleware.RequireAuth(cfg), handler.LifeAgentsCitationDetail(cfg))
			lifeAgents.GET("/:id/knowledge", middleware.RequireAuth(cfg), handler.LifeAgentsKnowledgeList(cfg))
			lifeAgents.POST("/:id/knowledge", middleware.RequireAuth(cfg), handler.LifeAgentsKnowledgeCreate(cfg))
			lifeAgents.PATCH("/:id/knowledge/:entryId", middleware.RequireAuth(cfg), handler.LifeAgentsKnowledgeUpdate(cfg))
			lifeAgents.DELETE("/:id/knowledge/:entryId", middleware.RequireAuth(cfg), handler.LifeAgentsKnowledgeDelete(cfg))
			...
			lifeAgents.GET("/:id/co-edit-events", middleware.RequireAuth(cfg), handler.LifeAgentsCoEditEvents(cfg))
			...
			lifeAgents.GET("/:id/growth-log", handler.LifeAgentsGrowthLog(cfg))
			lifeAgents.GET("/:id/manage/growth-log", middleware.RequireAuth(cfg), handler.LifeAgentsManageGrowthLog(cfg))
			lifeAgents.POST("/:id/growth-log/mark-seen", middleware.RequireAuth(cfg), handler.LifeAgentsGrowthLogMarkSeen(cfg))
