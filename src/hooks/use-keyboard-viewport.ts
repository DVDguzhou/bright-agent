...
function measureViewport(nativeKeyboardInset: number): ViewportBox {
  ...
  if (!vv) {
    const height =
      nativeKeyboardInset > 0 ? Math.max(0, layoutHeight - nativeKeyboardInset) : layoutHeight;
  ...
  if (insetFromVv >= KEYBOARD_INSET_THRESHOLD) {
    return { height: vvHeight, offsetTop: vvOffsetTop };
  }
...
export function useKeyboardViewport(mobileEnabled: boolean) {
  ...
  return { viewportBox, containerStyle };
}
