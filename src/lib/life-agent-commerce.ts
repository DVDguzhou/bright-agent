...
/** 是否展示购买相关 UI（价格、已购买 Tab、提问包入口等） */
export function lifeAgentShowsPurchaseUi(): boolean {
  if (!LIFE_AGENT_SHOW_PURCHASE_UI || LIFE_AGENT_UNLIMITED_CHAT) return false;
  if (typeof window !== "undefined" && iosBlocksInAppDigitalPurchase()) return false;
  return true;
}
