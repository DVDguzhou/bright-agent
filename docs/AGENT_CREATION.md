...
3. 启动本地 Agent：
   ```bash
   node scripts/platform/seller-agent-example.mjs
   ```
4. 启动隧道客户端（另一个终端）：
   ```bash
   export PLATFORM_URL="http://localhost:3000"
...
**实现方式：**
- **脚本编排**：在自有脚本中依次申请 token、调用各 Agent（见 `scripts/workflows/invoke-video-pipeline.mjs`）
...
- [卖方 Agent 最小示例](../scripts/platform/seller-agent-example.mjs)
- [隧道客户端](../scripts/platform/tunnel-client.mjs)
